...
// Target string to replace (original settings panel layout)
const targetText = `        <!-- School Settings View Panel -->
        <div id="settings-panel" class="settings-container hidden">
          ...
          <div class="settings-grid">
            <!-- 1. Pending Approvals -->
            <div class="settings-card" id="card-pending-approvals">
              <h4>👥 가입 승인 대기 목록</h4>
            ...
            <!-- 4. Class Setup -->
            <div class="settings-card" id="card-class-setup">
              <h4>🏫 학년/학급 생성 및 담임 지정</h4>
              <form id="class-setup-form">
                ...
                <button type="submit" class="btn btn-primary btn-block">학급 생성/수정 저장</button>
              </form>
            </div>

            <!-- 1.5 Approved Students Shortcut -->
            <div class="settings-card" style="cursor:pointer;" onclick="openStudentMgmtOverlay()">
              <h4 style="margin-bottom:0.5rem; color:var(--primary-color);">🎓 가입 완료 학생 관리</h4>
              <p style="font-size:0.85rem; color:var(--text-muted); margin-bottom:1rem;">가입 완료된 학생들의 아이디/비밀번호 조회 및 개별/선택 일괄 삭제 관리를 새 창으로 엽니다.</p>
              <button type="button" class="btn btn-outline btn-block">관리 창 열기</button>
            </div>
            
            <!-- 2.5 Teacher Management Shortcut -->
            <div class="settings-card" style="cursor:pointer;" onclick="openTeacherMgmtOverlay()">
              <h4 style="margin-bottom:0.5rem; color:var(--primary-color);">👩‍🏫 교사 등록 및 관리</h4>
              <p style="font-size:0.85rem; color:var(--text-muted); margin-bottom:1rem;">신규 교사 등록, 과목 변경, 계정 및 비밀번호 설정, 교사 목록 조회를 새 창으로 엽니다.</p>
              <button type="button" class="btn btn-outline btn-block">관리 창 열기</button>
            </div>

            <!-- 4.5 Class Management (Full Width) -->
            <div class="settings-card" id="card-class-management" style="grid-column: 1 / -1;">
              <h4 style="margin-bottom: 1rem;">🏫 학년/학급 생성 및 담임관리</h4>
              <div class="table-responsive">
                <table class="logs-table">
                  <thead>
                    <tr>
             ...
             <!-- 5. Account Settings -->
            <div class="settings-card" id="card-account-settings">
              <h4>🔐 담당자 계정 정보 변경</h4>
              <form id="admin-credentials-form">
                ...
                <button type="submit" class="btn btn-primary btn-block">계정 정보 변경 적용</button>
              </form>
            </div>

            <!-- 7. Holidays Manager -->
            <div class="settings-card" id="card-holidays-manager">
              <h4>📅 학업 휴일 설정 및 등록 (휴업일)</h4>
              <form id="holiday-setup-form">
                <div class="form-group">
                  <label>날짜</label>
                ...
                <div class="form-check" style="margin-bottom: 1rem;">
                  <input type="checkbox" id="chk-is-holiday" checked>
                  <label for="chk-is-holiday">휴업일 지정 (체크 시 시간표 등록 및 표시 제한)</label>
                </div>
                <button type="submit" class="btn btn-primary btn-block">휴일 등록하기</button>
              </form>
              <div class="settings-list-container" id="admin-holidays-list-ui"></div>
            </div>
          </div>
        </div>`;
...
const normReplace = normalize(replacementText);

if (normHtml.includes(normTarget)) {
  const updatedHtml = normHtml.replace(normTarget, normReplace);
  fs.writeFileSync(filePath, updatedHtml, 'utf8');
  console.log('SUCCESS: settings layout replaced successfully.');
} else {
  console.error('ERROR: Target text not found in index.html.');
  // Print some snippets to debug
